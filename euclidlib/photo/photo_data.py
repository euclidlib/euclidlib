...
def parse_file_name(file_name):
    ...
    if res_match:
        ...
        if len(res_match.groups()) == n_match:
            ...
            res_match_time_stamp = re.match(pattern_time_stamp, fname_info['time_stamp'])
            ...
            raise IndexError(
                ...
                + f' {len(res_match.groups())} instead of'
                + f' {n_match}'
            )
    #else:
    ...
    def init_from_fits(self, file_path, ignore_thunit=False):
        ...
        if not ignore_thunit:
            if "THUNIT" in hdu_list[0].header:
                ...
                raise KeyError(
                    f"Key THUNIT not found in header of input file {input_file}"
                )
        else:
            unit_str = None
    ...
    def check_consistency_scales(self, x, key):
        ...
        if len(x) != len(y):
            raise PhotoDataError(
                f'column \'{key}\': inconsistent lengths '
                + f'{len(x)}, {len(y)}'
            )
        ...
        for xx, yy in zip(x, y):
            ...
            if np.abs(f) > self.prec_rel:
                 raise PhotoDataError(
                    f'values {xx}, {yy} are inconsistent'
                )
    ...
    def from_fits(cls, file_path, corr_str='ShearShear'):
        ...
        for index in range(1, len(hdu_list)):
            ...
            if corr_str == 'ShearShear':
                ...
                raise ValueError(
                    f'Correlation {corr_str} not yet implemented'
                )
